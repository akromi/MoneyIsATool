-- Record where a trade's price actually came from.
--
-- 0006 hardcoded price_source to 'seeded', which was true when every price was
-- generated. Now that a teacher can enter a real close, that constant quietly
-- mislabels the trades made against real numbers — and sim_trades is
-- append-only, so a wrong label there is permanent. The caller passes the
-- source of the price it used, the way it already passes the price itself.

-- The old signature has to go, or both remain callable and PostgREST has to
-- guess which one a call meant.
drop function if exists public.sim_place_trade(uuid, uuid, text, numeric, numeric, date, text, text);
...
create or replace function public.sim_place_trade(
  ...
  p_reason_code text,
  p_reason_text text,
  p_price_source text default 'seeded'
)
returns uuid
language plpgsql
security definer
...
begin
  ...
  values
    (p_student_id, p_instrument_id, p_side, p_quantity, p_price, p_price_as_of, 'end_of_day',
     coalesce(nullif(btrim(p_price_source), ''), 'seeded'), p_reason_code, p_reason_text)
  returning id into v_id;

  return v_id;
end $$;

revoke execute on function public.sim_place_trade(uuid, uuid, text, numeric, numeric, date, text, text, text)
  from public, anon, authenticated;

comment on function public.sim_place_trade is
  'Validates and records one trade atomically, under a lock on the student row, so two requests arriving together cannot both spend the same cash. Records the source of the price it was given.';
